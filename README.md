...
本系統各元件分屬三個不同層次，服務不同角色、回答不同問題：

```
第一層｜GHG 盤查基礎設施（本專案核心）
  Pipeline → ETL → Routes → 碳排計算 → BigQuery
  問的是：「這批排放數字正不正確？」
  使用者：碳盤查人員、資料工程師

第二層｜稽核驗證
  HTML Control Tower + audit_trail
  問的是：「數字可不可信、有沒有被篡改？」
  使用者：ISAE 3410 稽核員、外部查驗機構

第三層｜管理決策（Looker Studio — 待實作）
  問的是：「要在哪裡改善？改善策略有沒有奏效？」
  使用者：永續長、物流主管、決策層
```

Looker Studio 不是盤查工具，而是把**已驗證的排放數據轉化為行動情報**的界面。例如：哪個承運商排放效率最差、哪條路線 CO₂/噸公里最高、換了更省油的車隊後月碳排是否下降。這個層次的問題必須建立在前兩層資料可信的基礎上才有意義。

> **這些數字最終去哪裡？** 本系統輸出的是 Scope 3 Category 4 的活動數據與稽核軌跡，後續需彙整進全公司 GHG 盤查報告，經第三方查驗機構（ISAE 3410）出具查驗聲明後，分別提交環境部氣候變遷署（碳費申報）或公告於台灣證交所永續報告書平台（ESG 揭露）。完整的申報鏈說明，包含台灣法規脈絡、查驗機構角色、兩條揭露路徑與申報週期，請見 [docs/reporting-chain.md](docs/reporting-chain.md)。
